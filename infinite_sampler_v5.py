"""
LTXInfiniteDirectorSamplerV5 — WAN-style pixel re-injection for identity consistency.

Builds on V4 (WAN-style overlap carry) and adds pixel re-injection:
after chunk N is decoded and trimmed, the last rendered pixel frame is injected as
an additional image guide at position 0 of chunk N+1's DirectorGuide call.

This mirrors how WAN I2V works — the actual rendered output of chunk N becomes the
first-frame anchor for chunk N+1, preserving identity (face, clothing, lighting)
across chunk boundaries rather than always reverting to the original static reference.

New widget: live_frame_strength (0.0-1.0, default 0.8)
  Controls how strongly the live last-frame guides chunk N+1 start.
  0.0 = disabled (same as V4), 1.0 = full lock.
"""
...
def _save_chunk_mp4(frames, fps, path):
    ...
    with _av.open(str(path), mode="w") as container:
        ...
        for pkt in stream.encode():
            container.mux(pkt)
    log.info("[LTXInfiniteDirectorSamplerV5] Saved: %s", path)
...
class LTXInfiniteDirectorSamplerV5:
    """
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "live_frame_strength": ("FLOAT", {"default": 0.8, "min": 0.0, "max": 1.0, "step": 0.05,
                                                  "tooltip": "WAN-style: last rendered frame of chunk N injected as image guide at chunk N+1 start. 0=off (same as V4)."}),
    ...
    DESCRIPTION = (
        "WAN-style overlap carry for LTX 2.3. Each chunk's start is extended back by "
        "carry_frames so the model generates the overlap with correct audio conditioning. "
        "No latent injection, no masks — true model-generated continuity."
    )
    ...
    def execute(
        ...
        ic_lora_name, ic_lora_strength, live_frame_strength,
        ...
        log.info("[LTXInfiniteDirectorSamplerV5] %s mode — %.1fs total", mode, total_duration)
        ...
        while t < end_second - 0.01:
            ...
            chunks.append((t, end))
            t = end
        log.info("[LTXInfiniteDirectorSamplerV5] %d chunk(s), carry_frames=%d (WAN-style overlap)", len(chunks), carry_frames)
        ...
        try:
            ...
            for _bseg in _td_pre.get("bgAudioSegments", []):
                ...
                try:
                    ...
                    log.info("[LTXInfiniteDirectorSamplerV5] BG pre-loaded: %s", _af)
                except Exception as _exc:
                    log.warning("[LTXInfiniteDirectorSamplerV5] BG pre-load failed: %s", _exc)
        except Exception as _exc:
            log.warning("[LTXInfiniteDirectorSamplerV5] BG pre-parse failed: %s", _exc)
        ...
        live_pixel_frame = None  # last decoded pixel frame of chunk N → injected as guide for chunk N+1
        ...
        for chunk_idx, (chunk_start, chunk_end) in enumerate(chunks):
            ...
            log.info(
                "[LTXInfiniteDirectorSamplerV5] Chunk %d/%d  %.2f→%.2f s  (overlap=%d fr back to %.3fs)  seed=%d",
            ...
            # ── WAN-style pixel re-injection: inject last frame of chunk N as guide at chunk N+1 start ──
            # Mirrors WAN I2V — actual rendered output anchors identity at each chunk boundary
            # rather than always reverting to the original static reference image.
            if live_pixel_frame is not None and chunk_idx > 0 and live_frame_strength > 0.0:
                try:
                    gd = dict(guide_data)
                    gd["images"]        = [live_pixel_frame] + list(gd.get("images", []))
                    gd["insert_frames"] = [0]               + list(gd.get("insert_frames", []))
                    gd["strengths"]     = [live_frame_strength] + list(gd.get("strengths", []))
                    guide_data = gd
                    log.info(
                        "[LTXInfiniteDirectorSamplerV5] Live frame injected at position 0 (strength=%.2f)",
                        live_frame_strength,
                    )
                except Exception as exc:
                    log.warning("[LTXInfiniteDirectorSamplerV5] Live frame injection failed: %s", exc)
            ...
            if carry_lat is not None and overlap_frames > 0:
                try:
                    ...
                    log.info(
                        "[LTXInfiniteDirectorSamplerV5] Carry injected: %d latent frames locked in Stage1, Stage2 free",
                        n_lat_carry,
                    )
                except Exception as exc:
                    log.warning("[LTXInfiniteDirectorSamplerV5] Carry injection failed: %s", exc)
            ...
            # Stage2 carry mask removed — any non-zero value here carries visual
            # artifacts (e.g. hallucinated subtitles) from chunk N into chunk N+1.
            # Stage1 injection + 0.42 denoise gives sufficient visual continuity.
            ...
            log.info("[LTXInfiniteDirectorSamplerV5] latent %s → decoded %s", lat_shape, list(frames.shape))
            ...
            try:
                ...
                log.warning("[LTXInfiniteDirectorSamplerV5] Chunk save failed: %s", exc)
            ...
            if overlap_frames > 0:
                ...
                log.info(
                    "[LTXInfiniteDirectorSamplerV5] Trimmed %d overlap frames from chunk %d (WAN-style)",
            ...
            if chunk_idx == 0:
                ...
                try:
                    ...
                    log.info(
                        "[LTXInfiniteDirectorSamplerV5] Color match chunk %d: "
                        "mean %s→%s std %s→%s",
                        chunk_idx + 1,
                    ...
                    log.warning("[LTXInfiniteDirectorSamplerV5] Color match failed: %s", exc)

            all_frames.append(frames)

            # Store last pixel frame for next chunk's live re-injection
            ...
            if isinstance(combined_audio, dict) and "waveform" in combined_audio:
                ...
                if overlap_frames > 0:
                    ...
                    log.info(
                        "[LTXInfiniteDirectorSamplerV5] Trimmed %d audio samples (overlap %d frames)",
        ...
        if all_bg_waveforms:
            try:
                ...
                log.info("[LTXInfiniteDirectorSamplerV5] Mixed BG audio (per-chunk, %d chunks)", len(all_bg_waveforms))
            except Exception as exc:
                log.warning("[LTXInfiniteDirectorSamplerV5] BG audio mix failed: %s", exc)
        ...
        if bg_audio is not None and "waveform" in bg_audio:
            try:
                ...
                log.warning("[LTXInfiniteDirectorSamplerV5] Wire BG audio mix failed: %s", exc)
        ...
        try:
            ...
            log.warning("[LTXInfiniteDirectorSamplerV5] Full video save failed: %s", exc)

        return (full_video, full_audio)


NODE_CLASS_MAPPINGS = {
    "LTXInfiniteDirectorSamplerV5": LTXInfiniteDirectorSamplerV5,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "LTXInfiniteDirectorSamplerV5": "LTX Infinite Director Sampler V5 (Pixel Re-Inject)",
}
