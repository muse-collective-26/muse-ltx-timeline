...
class LTXInfiniteDirectorSampler:
    """
        ...
        return {
            "required": {
                ...
                "carry_frames":     ("INT",   {"default": 9,    "min": 1,    "max": 64,   "step": 1,
                                              "tooltip": "Pixel frames passed as latent context to next chunk (9 = 2 latent frames)."}),
    ...
    def execute(
        ...
        for chunk_idx, (chunk_start, chunk_end) in enumerate(chunks):
            ...
            # Use same seed for all chunks — different seeds create different pose attractors
            # that cause visible jumps at boundaries regardless of carry frame injection.
            chunk_seed = seed
            s2_seed = seed - 1
            ...
            # Director is called at custom_width/height — that IS the Stage 1 resolution.
            # The spatial upscaler 2× inside Stage 2 produces the final 2× output.
            fr = int(frame_rate)
            s_fr = int(chunk_start * frame_rate)
            e_fr = int(chunk_end * frame_rate)
            # Stage 1 runs at half resolution; upscaler 2× restores to target resolution.
            ...
            # n_prev_px = pixel frame count; convert to latent frames for mask sizing.
            # LTX temporal compression: latent_frames = (pixel_frames - 1) // 8 + 1
            n_prev_px = carry.shape[0] if carry is not None else 0
            n_prev = (n_prev_px - 1) // 8 + 1 if n_prev_px > 0 else 0  # latent frame count
            ...
            if has_audio:
                # Strip our carry noise_mask before AV separation — LTXVSeparateAVLatent
                # expects the mask to span both video+audio frames (masks[0] and masks[1]);
                # our video-only mask causes an IndexError when it tries to read masks[1].
            ...
            # WAN-style pixel carry: force-replace the first carry_frames decoded pixels
            # with the exact pixel frames from the previous chunk. This guarantees
            # pixel-perfect continuity at the carry boundary — the same approach WAN
            # InfiniteTalk uses (copy decoded pixels, never re-generate them).
            if chunk_idx > 0 and carry is not None:
                ...
                log.info("[LTXInfiniteDirectorSampler] Pixel-replaced %d carry frames (WAN-style, chunk %d)", n_replace, chunk_idx)
        ...
        # Mix in background audio from timeline bgAudioSegments
        try:
            import json as _json
            import torchaudio as _ta
            td = _json.loads(timeline_data) if timeline_data and timeline_data.strip() not in ("", "{}") else {}
            bg_segs = td.get("bgAudioSegments", [])
            tl_bg_vol = float(td.get("bgAudioVolume", 1.0))
            import folder_paths as _fp
            import soundfile as _sf
            for bg_seg in bg_segs:
                af = bg_seg.get("audioFile", "")
                if not af:
                    continue
                audio_path = None
                for base in [_fp.get_input_directory(), os.path.join(_fp.get_input_directory(), "whatdreamscost")]:
                    candidate = os.path.join(base, os.path.basename(af))
                    if os.path.exists(candidate):
                        audio_path = candidate
                        break
                if not audio_path:
                    log.warning("[LTXInfiniteDirectorSampler] BG audio file not found: %s", af)
                    continue
                try:
                    data, sr = _sf.read(audio_path, dtype="float32", always_2d=True)
                    bg_w = torch.from_numpy(data.T).unsqueeze(0)
                    if sr != audio_sample_rate:
                        bg_w = _ta.functional.resample(bg_w, sr, audio_sample_rate)
                    lip_w = full_audio["waveform"]
                    lip_len = lip_w.shape[-1]
                    trim_samples = int(bg_seg.get("trimStart", 0) / frame_rate * audio_sample_rate)
                    if trim_samples > 0:
                        bg_w = bg_w[..., trim_samples:]
                    ...
                    mixed = lip_w + bg_w.to(lip_w.device, lip_w.dtype) * tl_bg_vol
                    full_audio = {"waveform": mixed, "sample_rate": audio_sample_rate}
                    log.info("[LTXInfiniteDirectorSampler] Mixed BG audio: %s vol=%.2f", af, tl_bg_vol)
                except Exception as exc:
                    log.warning("[LTXInfiniteDirectorSampler] Failed to mix BG audio %s: %s", af, exc)
        except Exception as exc:
            log.warning("[LTXInfiniteDirectorSampler] BG audio mix error: %s", exc)

        # Cross-fade at chunk boundaries using crossfade_frames (independent of carry_frames)
        ...
        if len(all_frames) > 1 and n_cf_global > 0:
            ...
            log.info("[LTXInfiniteDirectorSampler] Cross-faded %d chunk boundaries (%d frames each)", len(all_frames) - 1, n_cf_global)
...
NODE_CLASS_MAPPINGS = {
    "LTXInfiniteDirectorSampler": LTXInfiniteDirectorSampler,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "LTXInfiniteDirectorSampler": "LTX Infinite Director Sampler",
}
