"""
LTXInfiniteDirectorSamplerV4 — WAN-style true overlap carry.

Instead of injecting carry frames as a latent hint (V1-V3 approach), V4 extends
each chunk's start window BACK by carry_frames so the model generates those frames
naturally with the correct audio conditioning. The carry region is then trimmed from
the output, leaving only new content — identical to WAN InfiniteTalk's overlap method.

No _inject_carry. No noise masks. No pixel replacement.
The same seed + same prompts + same image guide produce visually consistent carry
frames on both sides of every boundary.
"""
...
def _save_chunk_mp4(frames, fps, path):
    ...
    with _av.open(str(path), mode="w") as container:
        ...
        for pkt in stream.encode():
            container.mux(pkt)
    log.info("[LTXInfiniteDirectorSamplerV4] Saved: %s", path)
...
class LTXInfiniteDirectorSamplerV4:
    """
    ...
    def execute(
        ...
        log.info("[LTXInfiniteDirectorSamplerV4] %s mode — %.1fs total", mode, total_duration)
        ...
        while t < end_second - 0.01:
            ...
            chunks.append((t, end))
            t = end
        log.info("[LTXInfiniteDirectorSamplerV4] %d chunk(s), carry_frames=%d (WAN-style overlap)", len(chunks), carry_frames)
        ...
        try:
            ...
            for _bseg in _td_pre.get("bgAudioSegments", []):
                ...
                try:
                    ...
                    log.info("[LTXInfiniteDirectorSamplerV4] BG pre-loaded: %s", _af)
                except Exception as _exc:
                    log.warning("[LTXInfiniteDirectorSamplerV4] BG pre-load failed: %s", _exc)
        except Exception as _exc:
            log.warning("[LTXInfiniteDirectorSamplerV4] BG pre-parse failed: %s", _exc)
        ...
        for chunk_idx, (chunk_start, chunk_end) in enumerate(chunks):
            ...
            log.info(
                "[LTXInfiniteDirectorSamplerV4] Chunk %d/%d  %.2f→%.2f s  (overlap=%d fr back to %.3fs)  seed=%d",
            ...
            if carry_lat is not None and overlap_frames > 0:
                try:
                    ...
                    log.info(
                        "[LTXInfiniteDirectorSamplerV4] Carry injected: %d latent frames locked in Stage1, Stage2 free",
                        n_lat_carry,
                    )
                except Exception as exc:
                    log.warning("[LTXInfiniteDirectorSamplerV4] Carry injection failed: %s", exc)
            ...
            log.info("[LTXInfiniteDirectorSamplerV4] latent %s → decoded %s", lat_shape, list(frames.shape))
            ...
            try:
                ...
                log.warning("[LTXInfiniteDirectorSamplerV4] Chunk save failed: %s", exc)
            ...
            if overlap_frames > 0:
                ...
                log.info(
                    "[LTXInfiniteDirectorSamplerV4] Trimmed %d overlap frames from chunk %d (WAN-style)",
            ...
            if chunk_idx == 0:
                ...
                try:
                    ...
                    log.info(
                        "[LTXInfiniteDirectorSamplerV4] Color match chunk %d: "
                        "mean %s→%s std %s→%s",
                        chunk_idx + 1,
                    ...
                    log.warning("[LTXInfiniteDirectorSamplerV4] Color match failed: %s", exc)
            ...
            if isinstance(combined_audio, dict) and "waveform" in combined_audio:
                ...
                if overlap_frames > 0:
                    ...
                    log.info(
                        "[LTXInfiniteDirectorSamplerV4] Trimmed %d audio samples (overlap %d frames)",
        ...
        if all_bg_waveforms:
            try:
                ...
                log.info("[LTXInfiniteDirectorSamplerV4] Mixed BG audio (per-chunk, %d chunks)", len(all_bg_waveforms))
            except Exception as exc:
                log.warning("[LTXInfiniteDirectorSamplerV4] BG audio mix failed: %s", exc)
        ...
        if bg_audio is not None and "waveform" in bg_audio:
            try:
                ...
                log.warning("[LTXInfiniteDirectorSamplerV4] Wire BG audio mix failed: %s", exc)
        ...
        try:
            ...
            log.warning("[LTXInfiniteDirectorSamplerV4] Full video save failed: %s", exc)

        return (full_video, full_audio)


NODE_CLASS_MAPPINGS = {
    "LTXInfiniteDirectorSamplerV4": LTXInfiniteDirectorSamplerV4,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "LTXInfiniteDirectorSamplerV4": "LTX Infinite Director Sampler V4 (WAN-Style)",
}
